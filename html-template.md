...
5. **Comment / Annotate Mode** (included by default after draft generation):
   - A second review affordance that sits next to inline editing, in a distinct accent color
   - Lets the viewer point at any element or selected text and leave a note for the agent
   - Collects notes into a side panel and emits a structured "Copy for agent" bundle
   - See "Comment / Annotate Mode Implementation" section below
...
## Comment / Annotate Mode Implementation

Two complementary review affordances ship by default. Inline editing (above) is **direct
manipulation** — the viewer edits text themselves. Comment mode is **point-and-instruct** — the
viewer points at something and tells the agent what to change. Inline editing wins for typos and
rewording; comment mode wins for the structural/visual asks `contenteditable` can't express
("make this bigger", "this overflows", "swap these two slides", "off-brand color"). Include both
by default unless the user asks for a locked/export-only file.

Design rules:

- **Distinct accent.** Use a clearly different color from the inline-edit toggle so the two modes
  read as separate (e.g. green comment vs. terracotta edit). Reveal both via the same hover hotzone.
- **Mutually exclusive with edit mode.** Entering comment mode turns edit mode off and vice versa —
  `contenteditable` and the comment click-capture conflict. Wire both entry points so toggling one
  disables the other (no recursive toggle loops).
- **Capture precise targets.** On click in comment mode, capture: the 1-based slide number
  (`el.closest('.slide')` index), a short CSS path up to `.slide`, a `tag.class` label, and the
  element's text (or, if the viewer has a non-collapsed selection, the exact selected text). Text
  selection is the high-value case — it lets the agent target a span mid-paragraph, not "the part
  that says X".
- **Strip transient classes.** The hover-highlight class (e.g. `cm-hover`) is on the element at click
  time — filter it out before building the selector/label or it leaks into the captured target.
- **Fixed-stage safe.** All overlays (hover outline, comment card, panel) are `position: fixed` and
  measured with `getBoundingClientRect`, so they align correctly under the transform-scaled stage —
  unlike layout audits, this needs no per-slide activation.
- **Zero-dependency feedback loop.** No server. Notes persist to `localStorage`; a **"Copy for agent"**
  button writes a structured Markdown bundle (grouped by slide, with selector + quoted text + the
  requested change) to the clipboard. The viewer pastes it back to the agent, which applies every
  edit in one pass.
- **Interactions:** toggle via a floating button or `C` key (skip when a textarea/input/contenteditable
  is focused); `Ctrl/⌘+Enter` commits a note, `Esc` closes the card then exits the mode. Clicking a
  note in the panel jumps to that slide.

A reference implementation (the `cm` controller, `cmPath`/`cmLabel`/`cmSlideNo` helpers, hover/click
capture, and the copy-bundle builder) ships in `comment-mode.html` — adapt its
palette variables to the deck's tokens.
