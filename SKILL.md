...
3. **Summarize** — Tell the user:
   ...
   - Comment mode is available: press C (or the comment button next to the edit toggle), click any element or select text to leave a note, then "Copy for agent" to paste structured feedback back into chat
   - Offer the natural post-draft actions: ask for revisions, edit text directly in the browser, leave comments for the agent, or export/share
